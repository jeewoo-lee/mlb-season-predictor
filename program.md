...
1. **Read the in-scope files**:
   ...
   - `data/train/team_states.csv` - training team-seasons, populated by `prepare.sh`.
...
Each case is a team's preseason or All-Star-break state with team context, schedule, park, projection, defense, durability, and player-level roster fields. The frozen test covers 2 seasons × 30 teams × 2 checkpoints = 120 cases (real MLB data, anonymized identifiers). Each team-state also carries a `roster` list in `agent.predict`, with 26-man-style players plus projected call-ups.
